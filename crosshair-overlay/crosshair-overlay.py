...
def on_activate(app):
    ...
    # 1. Initialize Layer Shell for this window
    Gtk4LayerShell.init_for_window(win)

    # 2. Set the Namespace (Crucial for niri config matching)
    Gtk4LayerShell.set_namespace(win, "crosshair-overlay")

    # 3. Set the Layer to OVERLAY
    Gtk4LayerShell.set_layer(win, Gtk4LayerShell.Layer.OVERLAY)

    # 4. Make it click-through (no keyboard/mouse focus)
    Gtk4LayerShell.set_keyboard_mode(win, Gtk4LayerShell.KeyboardMode.NONE)

    # 5. Visuals: Transparent background
